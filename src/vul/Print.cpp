...
const char *vul::Print::deviceTypeName(VkPhysicalDeviceType t)
{
  switch (t) {
...
void vul::Print::printQueueFamilies(const int tab, const VkPhysicalDevice physicalDevice)
{
...
std::string vul::Print::VkQueueFlags_toStr(const VkQueueFlags flags)
{
  std::string ret;
...
void vul::Print::printPhysicalDeviceProperties(const uint32_t deviceIndex, const VkPhysicalDevice physicalDevice)
{
...
void vul::Print::loadInstanceExtensions(const bool print, std::unordered_set<std::string> *targetExtensions)
{
  ...
  for (const VkExtensionProperties &ext : extensions) {
    if (const size_t len = std::strlen(ext.extensionName); maxLength < len) {
      maxLength = len;
    }
    if (targetExtensions) {
